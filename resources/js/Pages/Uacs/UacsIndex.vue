...
const add_uacs_dialog = ref(false)

// add year
const add_uacs_form = useForm({
    uacs_type: '',
    uacs_group: '',
    uacs_code: '',
    object_code: '',
...
const submit = () => {
    add_uacs_form.post(route("uacs.store"), {
        onSuccess: () => {
            add_uacs_form.reset(); // Reset the form after successful submission
            add_uacs_dialog.value = false;
        },
    });
};
// end add year
    ...
    <AuthenticatedLayout>
        ...
        <div>
            ...
            <!-- modals  -->
             <div>
                <!-- add year  -->
                <v-dialog v-model="add_uacs_dialog" persistent max-width="600">
                        ...
                        <v-card-text>
                            <div>
                                <form @submit.prevent="submit">
                                    <div class="mb-4">
                                        <div>
                                            <InputLabel for="uacs_type" value="UACS Type" required="true"/>
                                            <SelectInput>
                                                <option value="" disabled>Select UACS Type</option>
                                                <option v-for="type in $page.props.uacs_types" :key="type.id" :value="type.id">{{ type.name }}</option>
                                            </SelectInput>
                                            <InputError class="mt-2" :message="add_uacs_form.errors.uacs_type" />
                                        </div>
                                    </div>
                                    <div class="mb-4">
                                        <div>
                                            <InputLabel for="uacs_group" value="UACS Group" required="true"/>
                                            <SelectInput>
                                                <option value="" disabled>Select UACS Group</option>
                                                <option v-for="group in $page.props.uacs_groups" :key="group.id" :value="group.id">{{ group.name }}</option>
                                            </SelectInput>
                                            <InputError class="mt-2" :message="add_uacs_form.errors.uacs_group" />
                                        </div>
                                    </div>
                                    <div class="mb-4">
                                        <div>
                                            ...
                                            <TextInput
                                                ...
                                                v-model="add_uacs_form.year"
                                                autocomplete="year"
                                                placeholder="Enter year."
                                            />
                                            <InputError class="mt-2" :message="add_uacs_form.errors.year" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="text-end">
                                        ...
                                        <v-btn
                                            ...
                                            variant="tonal"
                                            type="submit"
                                            :class="{ 'opacity-25': add_uacs_form.processing }"
                                            :loading="add_uacs_form.processing"
                                        ></v-btn>
                                    </div>
                                </form>
                            </div>
